...
void HERAII()
{
	...
	//**************************************************************************
	TH1F *hpt=new TH1F("hpt","K0s transverse momenta distribution, 2007pRP",10,0,5);
	hpt->GetYaxis()->SetTitle("Entries");
	hpt->GetXaxis()->SetTitle("P_t, GeV");
	TH1F *myobject;
	TClonesArray a("TH1F",10);
	new(a[1]) TH1F("pt1","K0s inv mass in particular pt region",60,450,552);
	new(a[2]) TH1F("pt2","K0s inv mass in particular pt region",60,450,552);
	new(a[3]) TH1F("pt3","K0s inv mass in particular pt region",60,450,552);
	new(a[4]) TH1F("pt4","K0s inv mass in particular pt region",60,450,552);
	new(a[5]) TH1F("pt5","K0s inv mass in particular pt region",60,450,552);
	new(a[6]) TH1F("pt6","K0s inv mass in particular pt region",60,450,552);
	new(a[7]) TH1F("pt7","K0s inv mass in particular pt region",60,450,552);
	new(a[8]) TH1F("pt8","K0s inv mass in particular pt region",60,450,552);
	new(a[9]) TH1F("pt9","K0s inv mass in particular pt region",60,450,552);
	new(a[10]) TH1F("pt10","K0s inv mass in particular pt region",60,450,552);

	TH1F *heta=new TH1F("heta","K0s pseudorapidity distribution, 2006p",6,-3,3);
	TClonesArray b("TH1F",10);
	new(b[1]) TH1F("eta1","K0s inv mass in particular eta region",60,450,552);
	new(b[2]) TH1F("eta2","K0s inv mass in particular eta region",60,450,552);
	new(b[3]) TH1F("eta3","K0s inv mass in particular eta region",60,450,552);
	new(b[4]) TH1F("eta4","K0s inv mass in particular eta region",60,450,552);
	new(b[5]) TH1F("eta5","K0s inv mass in particular eta region",60,450,552);
	new(b[6]) TH1F("eta6","K0s inv mass in particular eta region",60,450,552);
	//***************************************************************************
	...
	for (Int_t i=0;i<nevents;i++)
	{
		...
		for(Int_t k=0;k<nv0;k++)
		{
			...
			if (coll2[k]>0.1) continue;  //.1              .12
			if(mass_pi_p<1.125) continue; //1.125        1.121
			hd1->Fill(2);
			...
			if (K0s_cand.GetPt_m()<0.3) continue;  //.3        .25
			hd1->Fill(3);
			...
			if ((K0s_cand.GetMass_m(4,4)<0.4777)||(K0s_cand.GetMass_m(4,4)>0.5177)) continue;   //.4777 .5177
			hphi->Fill(phi);
			hd1->Fill(7);
			list_k0[cand_k0]=k;
			cand_k0++;
		...
		for(Int_t k=0;k<cand_k0;k++)
		{
			...
			//if ((1000*K0s_candtemp.GetMass_m(4,4)<450)||(1000*K0s_candtemp.GetMass_m(4,4)>552)) cout<<"!! "<<1000*K0s_candtemp.GetMass_m(4,4)<<endl;
			hinv_base->Fill(1000*K0s_candtemp.GetMass_m(4,4));
			Float_t pt_step=0.5;
			for(int u=1;u<11;u++)
			{
				if ((K0s_candtemp.GetPt_m()>(u-1)*pt_step)&&(K0s_candtemp.GetPt_m()<u*pt_step)) ((TH1F *)a.At(u))->Fill(1000*K0s_candtemp.GetMass_m(4,4));
			}
			Float_t eta_step=1.0,
				in_eta=-3.0;
			for(int e=1;e<7;e++)
			{
				Double_t	eta=K0s_candtemp.GetEta_m(),
						eta_min=in_eta+(e-1)*eta_step,
						eta_max=in_eta+e*eta_step;

				if ((eta>eta_min)&&(eta<eta_max)) ((TH1F *)b.At(e))->Fill(1000*K0s_candtemp.GetMass_m(4,4));
			}
		}
		...
		for(Int_t k=0;k<cand_k0-1;k++)
		{
			for(Int_t l=k+1;l<cand_k0;l++)
			{
				...
				hd2->Fill(3);
    				h->Fill(reso_mass);
				h1->Fill(reso_mass);
				Double_t K0sK0s_eta=K0sK0s.GetEta_m();
				if (K0sK0s_eta<-1) h_eta1II->Fill(reso_mass);
				if ((K0sK0s_eta>=-1)&&(K0sK0s_eta<=1)) h_eta2II->Fill(reso_mass);
				if (K0sK0s_eta>1) h_eta3II->Fill(reso_mass);
			}
		}
	}

	//TFile *f1 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/04-07_2.root","recreate");
	TFile *f1 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/06p-07p_1.root","recreate");
	//TFile *f1 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/2005pRP.root","recreate");
	...
	{
		cout<<sign1[e-1]<<endl;
		Int_t ssd=(int)sign1[e-1];
		cout<<ssd<<"   ssssss"<<endl;
		heta->SetBinContent(e,ssd);
	}
	heta->Write();
	//***********************************************************************************
