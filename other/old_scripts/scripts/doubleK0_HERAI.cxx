...
int main()
{
	...
	myChain->Add("/data/zenith226a/libov/data/96_99/v0_mini00p_0.root");
	...
	for(Int_t i=0;i<nevents;i++)
	{
		...
		// K0s selecting
		if (Nv0<=1) continue;  // at least two V0 required
		hdebug->Fill(7);
		for(Int_t j=0;j<Nv0;j++)
		{
			...
			if (Tsecvtx_collin2>0.3) continue;
			hdebug1->Fill(7);
			if (Tsecvtx_collin3>0.3) continue;
			hdebug1->Fill(8);
		...
		for(Int_t k=0;k<cand_k0-1;k++)
		{
			for(Int_t l=k+1;l<cand_k0;l++)
			{
				...
				hdebug2->Fill(2);
				Daughter K0s_cand1(P1_x[k01],P1_y[k01],P1_z[k01]);
				Daughter K0s_cand2(P2_x[k02],P2_y[k02],P2_z[k02]);
	...
	TFile *f1 =new TFile("/data/zenith226a/libov/results/recent/K0sK0sHERAI_4.root","recreate");
	h1->Write();
	hinv_base->Write();
	hdebug->Write();
	hdebug1->Write();
