...
int main()
{
	TFile *f1 =new TFile("histograms.root/DoubleK0stest.root");
	Double_t 	coll2[7],
			coll2_err[7],
			signal1[7],
			signal2[7],
	...
	for(Int_t cut_loop=0;cut_loop<5;cut_loop++)
	{

		Double_t 	coll2_cut=0.15-cut_loop*0.025;

		coll2_err[cut_loop]=0;
		coll2[cut_loop]=coll2_cut;
		cout<<"iterator: "<<cut_loop<<"  Collinearity2< "<<coll2_cut<<endl;
		char 	t1[30],
			t2[30],
			t3[30];
	...
	//	htemp2->Fit("fexp1","R+");
		...
		for(int i=0;i<7;i++)
		cout<<signal1[i]<<" "<<signal2[i]<<" "<<coll2[i]<<endl;
		TCanvas *c=new TCanvas("c","ss",1000,1000);
		TGraph *gr1=new TGraphErrors(7,coll2,signal1,coll2_err,signal1_err);
		TGraph *gr2=new TGraphErrors(7,coll2,signal2,coll2_err,signal2_err);
		TGraph *gr3=new TGraph(7,coll2,significance1);
		TGraph *gr4=new TGraph(7,coll2,significance2);
