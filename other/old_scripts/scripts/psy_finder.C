...
void doubleK0_HERAI_fdr()
{
	...
	//myChain->SetBranchAddress("Tt1_the",Tt1_the);
	//myChain->SetBranchAddress("Tt2_the",Tt2_the);
	//myChain->SetBranchAddress("Tt1_eta",Tt1_eta);
	//myChain->SetBranchAddress("Tt2_eta",Tt2_eta);
	...
	TH1F *h_eta1=new TH1F("h_eta1","K0sK0s",200,0.8,5);
	TH1F *h_eta2=new TH1F("h_eta2","K0sK0s",200,0.8,5);
	TH1F *h_eta3=new TH1F("h_eta3","K0sK0s",200,0.8,5);
	...
	for(Int_t i=0;i<nevents;i++)
	{
		...
		// K0s selecting
		//if (Nv0psy<=1) continue;  // at least two V0s required
		hdebug->Fill(7);
		...
		/*for(Int_t k=0;k<cand_k0-1;k++)
		{
			for(Int_t l=k+1;l<cand_k0;l++)
			{
				...
				if (K0sK0s_eta<-1) h_eta1->Fill(reso_mass);
				if ((K0sK0s_eta>=-1)&&(K0sK0s_eta<=1)) h_eta2->Fill(reso_mass);
				if (K0sK0s_eta>1) h_eta3->Fill(reso_mass);
			}
		}*/
	}

	TFile *f1 =new TFile("/data/zenith226a/libov/test.root","recreate");
	h1->Write();
	h2->Write();
	hinv_base->Write();
	hinv1->Write();
