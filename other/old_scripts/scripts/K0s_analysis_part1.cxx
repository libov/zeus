...
void analysis()
{
	...
	cout<<"Add files to chain"<<endl;

	ZaconChainFactory_v02_07p::addToZaconChain(myChain);
	...
	myChain->SetBranchAddress("Tsecvtx",Tsecvtx);
	...
	Float_t		p1[80][3],	// momenta of 1st track
			...
			e_pz_sira,		// E-pz calculated both from hadronic system and electron, Sinistra!
...
//-----------------------------------------------------------------//
	...
	Int_t	file_num=0;
	Int_t temp=0;
	temp=sprintf(name,"comtest1_%d.root",file_num);
	...
	tree->Branch("e_pz_sira",&e_pz_sira,"e_pz_sira/F");
	...
	tree->Branch("cal_empz",&cal_empz,"cal_empz/F");
...
//------ Loop over events -------//
	...
	for(Int_t i=0;i<nevents;i++)
	{
		...
		{
			cout<<"Should be "<<loc_nv0<<" v0's  in "<<name<<endl;
			loc_nv0=0;
			tree->Print();
			tree->Write();
			cout<<"tree written"<<endl;
			delete tree;
			cout<<"tree pointer deleted"<<endl;
			f2->Close();
			cout<<"file closed"<<endl;

			//-------------------- new file -------------------//
			file_num++;
			temp=sprintf(name,"comtest_%d.root",file_num);
	...
	//------DIS event selection------//
		...
		if (Sincand!=0)
		{
			cout<<"Sincand= "<<Sincand<<endl;
			cout<<"Empz_e= "<<Empz_e<<endl;
			cout<<"Empz_had= "<<Empz_had<<endl;
			cout<<"Cal_empz= "<<Cal_empz<<endl;
		}*/
	...
	//------ (soft) K0s selection------//
		...
		for(Int_t j=0;j<Nv0lite;j++)
		{
			...
			Float_t t2p=t2.GetP();
			Float_t mass_pi_p=0;
			if (t1p>=t2p)  //first track proton(antiproton); second track pion_minus(pion_plus)
			{
				mass_pi_p=K0s_cand.GetMass_m(6,4);
				//if(Tq1[j]>0) cout<<"Lambda"<<endl;
				//if(Tq1[j]<0) cout<<"ALambda"<<endl;
			}
			if (t1p<t2p) //first track pion_minus(pion_plus); first track proton(antiproton);
			{
	...
	//---- Tree filling -----//
		...
		if (Sincand>0)
		{
			...
			box_x=x_srtd;
			box_y=y_srtd;
			e_pz_sira=EminPz_Evt;
			yel=Siyel[0];
			yjb=Siyjb[0];
		}
		sincand=Sincand;
		//cout<<event<<" @ "<<runnr<<endl;
