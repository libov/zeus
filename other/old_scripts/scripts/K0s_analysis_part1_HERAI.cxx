...
void analysis()
{
	...
	//------K0s selection------//
		...
		for(Int_t j=0;j<Nv0;j++)
		{
			...
			Daughter t1(V0p3[j][0],V0p3[j][1],V0p3[j][2]);
			Daughter t2(V0p3[j][0],V0p3[j][1],V0p3[j][2]);
		...
		if (cand_k0!=Nv0) cout<<"ERRRRRRROOOOOOOOOOOOORRRRRRRRR!!!!!!!!"<<endl;
	...
	TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_trees/heraI_test.root","recreate");
	tree->Write();
	f2->Close();
