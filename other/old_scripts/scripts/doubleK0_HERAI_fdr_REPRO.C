...
void doubleK0_HERAI_fdr()
{
	...
	for(Int_t i=0;i<nevents;i++)
	{
		...
		for(Int_t j=0;j<Nv0;j++)
		{
			...
			hdebug1->Fill(6);
			//if ((V0dlen[j]<2)||(V0dlen[j]>30)) continue;
			hdebug1->Fill(7);
			...
			//if ((Mpipi[j]<0.488)||(Mpipi[j]>0.508)) continue;
...
scal_fact=K0s_cand2.GetPx()*K0sK0s.GetPx_m()+K0s_cand2.GetPy()*K0sK0s.GetPy_m()+K0s_cand2.GetPy()*K0sK0s.GetPy_m();
	...
	TFile *f1 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/K0K0_HERAI_REPRO.root","recreate");
	h1->Write();
	h2->Write();
	hinv_base->Write();
	hinv1->Write();
