...
Double_t rel_breit_wigner_bg(Double_t *x,Double_t *par)
{
	...
	//return par[0]*exp*pow+bin_width*(par[5]*bw1+par[8]*par[5]*bw2+(1/4.0)*par[5]*bw3);


	//return par[0]*exp*pow+bin_width*(par[5]*bw1+par[8]*par[5]*bw2+par[11]*bw3+par[15]*bw4);
	//return par[0]*exp*pow+bin_width*(par[5]*bw1+par[8]*par[5]*bw2);
...
void fit(Int_t	number,TH1F* htemp2,Double_t ratio=0.9,Double_t f0_width=0.15,Double_t f0_mass=1.695,bool fix_f0_mass=false,bool fix_f0_width=false,bool fix_ratio=false)
{
	...
	htemp2->GetYaxis()->SetLabelSize(0.03);
	htemp2->Draw();
	// ---------------------------------------------------------------------//
	//TF1 *fsum=new TF1("fsum",rel_breit_wigner_bg,1.07,2.7,16);
	TF1 *fsum=new TF1(fsum_name,rel_breit_wigner_bg,1.04,2.705,13);
	fsum->SetLineColor(2);
...
//TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_tight.root");
//TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_tight1.root");
//TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_tight3.root");
TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_basic.root");
	TH1F *hmain = (TH1F*)f2->Get("h2");
	//TH1F *hmain = (TH1F*)f2->Get("h2z_1");
	...
	f0mass_initial=1.708;
	//f0mass_initial=1.714;

	f0width_initial=0.137;
	//f0mass_initial=1.724;
/*
