...
Double_t rel_breit_wigner_bg(Double_t *x,Double_t *par)
{
	...
	Double_t		beta=par[14];
	Double_t		gamma=par[15];

	Double_t		omega=par[16];
	Double_t		delta=par[17];
	...
	Double_t		Re_1=beta*cos(omega);
	Double_t		Im_1=beta*sin(omega);

	Double_t		Re_2=gamma*cos(delta);
	Double_t		Im_2=gamma*sin(delta);


	ReTensor=Re_1*ReBW[1]-Im_1*ImBW[1]+Re_2*ReBW[2]-Im_2*ImBW[2]+ReBW[3];
	ImTensor=Re_1*ImBW[1]+Im_1*ReBW[1]+Re_2*ImBW[2]+Im_2*ReBW[2]+ImBW[3];
...
int main()
{
	...
	fsum->SetParName(14,"beta");
	fsum->SetParName(15,"gamma");
	fsum->SetParName(16,"omega");
	fsum->SetParName(17,"delta");
	...
	fbw1->SetParameters(par[3],par[4],par[11]*(par[14]**2));
	fbw1->Draw("same");

	TF1 *fbw2=new TF1("fbw2",rel_breit_wigner,0,10,4);
	fbw2->SetParameters(par[5],par[6],par[11]*(par[15]**2));
	fbw2->Draw("same");

	TF1 *fbw3=new TF1("fbw3",rel_breit_wigner,0,10,4);
	fbw3->SetParameters(par[7],par[8],par[11]);
	fbw3->Draw("same");
