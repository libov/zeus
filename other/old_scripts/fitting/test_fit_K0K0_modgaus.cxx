...
{
	// par[0] ---> par[9]  reserved for background
	// par[10] --->        breit-wigners
	...
	Double_t 	exp_mod=TMath::Exp((-1)*a*a/(2*sigma**2+par[13]*a));
	...
	return bg+bin_width*(par[5]*bw1+par[8]*par[5]*bw2+par[11]*bw3);
...
void fit(Int_t	number,TH1F* htemp2,Double_t ratio=0.9,Double_t f0_width=0.15,Double_t f0_mass=1.695,bool fix_f0_mass=false,bool fix_f0_width=false,bool fix_ratio=false)
{
	...
	TF1 *fsum=new TF1(fsum_name,rel_breit_wigner_bg,1.05,2.7,14);
	fsum->SetLineColor(2);
	...
	TF1 *fbg=new TF1("fbg",bg,1.1,2.7,4);
	par[0]=5000/corr;
	//par[0]=2000;
	par[1]=1.2;
	par[2]=0.7;
	...
	TF1 *fbg=new TF1(fbg_name,bg,1.05,2.3,4);
	fbg->SetParameters(par[0],par[1],par[2],par[13]);
	fbg->SetLineColor(3);
	fbg->Draw("same");
...
TFile *f2 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_basic.root");
	//TH1F *hmain = (TH1F*)f2->Get("hk0k0_4");
