...
Double_t rel_breit_wigner_bg(Double_t *x,Double_t *par)
{
	...
	Double_t			bw4=bw*ph_space;


	Double_t	bin_width=par[12];

	return bg+bin_width*(par[5]*bw1+par[5]*par[8]*bw2+par[11]*bw3+par[2]*bw4);
}
...
TFile *f1 =new TFile("/data/zenith226a/libov/results/K0sK0s_hist/HERAII_basic_substr.root");
	...
	CreateCanvas("K0K0");
	TCanvas *canv1=gROOT->FindObject("K0K0");
	Double_t par[25];
	...
	fsum->SetParName(2,"f0(980) const");
	...
	fsum->SetParName(13,"Exponent");
	fsum->SetParName(15,"Coeff linear");
	fsum->SetParName(16,"Coeff quadr");
	fsum->SetParName(20,"Bg norm");
	fsum->SetParName(21,"threshold power");
	...
	//fbg_init->FixParameter(15,par[15]);
	//fbg_init->FixParameter(16,par[16]);
	for(int k=17;k<20;k++)
	{
		fbg_init->FixParameter(k,par[k]);
	}
	fbg_init->SetLineColor(3);
	htemp2->Fit("fbg_init","R+");
	fbg_init->GetParameters(&par[0]);
	//par[20]=par[20]/corr;
	TF1 *fbg_init1=new TF1("fbg_init1",rel_breit_wigner_bg,0.995,2.7,22);
	fbg_init1->SetParameters(&par[0]);
	fbg_init1->SetLineColor(6);
	fbg_init1->Draw("same");

	par[2]=5*7000/corr_area;
	par[5]=8000/corr_area;
	par[11]=4000/corr_area;

	par[1]=0.05;//width of 980
	...
	//htemp2->Fit("fsum","NQR");
	//htemp2->Fit("fsum","R+");
	//gMinuit->Command("SCAN 16 100 -10 10");
	//gMinuit->Command("SCAN 17 100 -30 50");
	...
	par[5]=0;
	par[11]=0;
	TF1 *fbg_c=new TF1("fbg_c",rel_breit_wigner_bg,0.995,2.7,22);
