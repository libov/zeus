...
def	run_zarah(first, last):
	...
	command="zarah-jobsub -q M -s run_zarah.sh createMiniNtuples create-NtupleList.sh "+ANALYSIS+"/env.sh "+SampleName+".txt"
	#print command
...
for tmpline in configFile:
	...
	if (Nevents == -1):
		...
		sys.exit(1)
	#----------
	#continue --> for test reason - when need to skip the following part
	#----------
	# -- prepare directories
	commands.getoutput("sh prepare-dir2.sh "+SampleName)
